...
class ExcelPreprocessor:
    """
    Валидация и очистка данных перевозчиков и перевозок из Excel-файла.
    Атрибуты:
        errors — список ошибок валидации (лист, строка, поле, описание).
        stats  — словарь со статистикой обработки.
    Методы:
        process           — валидация обоих листов, возврат очищенных DataFrame.
        get_report        — формирование отчёта о предобработке.
        _is_empty         — проверка значения на пустоту.
        _parse_datetime   — безопасный парсинг даты/времени.
        _parse_int        — безопасный парсинг целого числа.
        _parse_float      — безопасный парсинг числа с плавающей точкой.
        _validate_carriers  — валидация листа перевозчиков.
        _validate_shipments — валидация листа перевозок.
    """
